...
class Class_Attention_multi(nn.Module):
    # taken from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/vision_transformer.py
    # with slight modifications to do CA 
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
...
class PSwinTransformer(nn.Module):
    """ Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False,depth_token_only=2,model_path=None,
                 num_stripes=2,pre_trained=None,multi_head=0, **kwargs):
        super().__init__()
        ...
        self.mlp_ratio = mlp_ratio
        if not embed_dim*8%num_stripes==0:
            print("the feature has 1024dim, and the num_stripes must can be divided")
            exit()
        self.num_stripes=num_stripes
        ...
        self.norm_list=[norm_layer(self.num_features)]
        for i in range(self.num_stripes):
            self.norm_list.append(norm_layer(int(self.num_features/self.num_stripes)))
        ...
        self.layer3 =copy.deepcopy(self.layers[2])
        self.layer4 =copy.deepcopy(self.layers[3])
    ...
    def forward_features(self, x,tdeep=0):
        B = x.shape[0]
        ...
        x=self.layers[0](x)
        x=self.layers[1](x)
        global_x=self.layers[2](x)
        global_x=self.layers[3](global_x)

        local_x=self.layer3(x)
        local_x=self.layer4(local_x)

        global_feat = self.norm(global_x)
        local_feat = self.norm2(local_x)
        ...
        x = self.avgpool(global_feat.transpose(1, 2))  # B C 1
        x = torch.flatten(x, 1)
        final_feats=[x]
        for i in range(self.num_stripes):
        ...
        if tdeep==0:
            return x+torch.mul(x,cls_tokens1[:,0,:]), x1+torch.mul(x1,cls_tokens2[:,0,:]), x2+torch.mul(x2,cls_tokens3[:,0,:])
        elif tdeep==1:
            return x+torch.mul(x,cls_tokens1[:,0,:]), torch.mul(x1,cls_tokens2[:,0,:]), torch.mul(x2,cls_tokens3[:,0,:])
        elif tdeep==2:
            return_feats=[]
            ...
            return_feats=[]
            for i in range(self.num_stripes+1):
                tmp_feat=torch.cat([final_feats[i],cls_tokens[i][:,0,:]],1)
                return_feats.append(tmp_feat)
            return return_feats

    def forward(self, x,tdeep=0):
        return self.forward_features(x,tdeep)
    ...
    def load_param(self, trained_path):
        ...
        print("load swin weights successfully")


if __name__ == '__main__':
    model = PSwinTransformer()
    model.eval()
    # model.load_param('/mnt/lustrenew/hezhiqun/.torch/models/jx_vit_base_p16_224-80ecf9dd.pth')
    for k, v in  model.named_parameters():
        v.requires_grad = False
    img = torch.randn(3, 3, 224, 224)
    preds = model(img,tdeep=2)
    print(len(preds))